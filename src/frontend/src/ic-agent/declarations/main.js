export const idlFactory = ({ IDL }) => {
  ...
  const DeterminateGatheringWorkPlan = IDL.Record({ 'weight' : IDL.Float64 });
  const EfficiencyGatheringWorkPlan = IDL.Record({ 'weight' : IDL.Float64 });
  ...
  const Work = IDL.Record({ 'workers' : IDL.Nat, 'units' : IDL.Nat });
